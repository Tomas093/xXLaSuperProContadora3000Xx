...
SILENT VISUAL CATALOG ASSOCIATION STEP
...
- Confirm a material only when the plan symbol visually matches the actual catalog image with effectively 100% certainty.
...
1. Perform the silent visual catalog association step:
   - Read every catalog "filename" + "component_name" pair.
   - Inspect the attached image for each filename.
   - Internally bind each actual catalog image to its exact "component_name".
   - Do not write this binding in the output.
   - Do not convert catalog images into verbal descriptions or feature lists as the matching source.
   - Use the actual images for matching, not any text description of them.
